                          ...
                          if (method == "linear") {
                            ...
                            if (!is.null(COV_)){
                                reduced_model = tryCatch(
                                    glm(y ~ LAG_[, LAcolnames[1:(length(LAcolnames) - 1)], drop=FALSE] + COV_, family = binomial(link = "logit")),
                                ...
                                reduced_model = tryCatch(
                                    glm(y ~ LAG_[, LAcolnames[1:(length(LAcolnames) - 1)], drop=FALSE], family = binomial(link = "logit")),
